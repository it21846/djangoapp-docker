pipeline {
    agent any
     ...
     environment {
            ...
            DOCKER_PREFIX = 'ghcr.io/it21846/djangoapp'
        }
    ...
    stages {
        stage('Build') {
            steps {
                ...
                git branch: 'k8s', url: 'https://github.com/it21846/djangoapp.git'
        ...
        stage('Test') {
            steps {
                sh '''
                    ...
                    sed -i 's#^DATABASE_URL.*#DATABASE_URL=sqlite:///.test.db#g' djangoapp/.env
                    ./manage.py test'''
            }
        }
        // stage('Deploy') {
